<div class="mb-5 bg-green-50 border-s-4 border-green-600 px-4 py-3" role="alert">
    <div class="flex">
        <svg class="flex-shrink-0 size-5 mt-0.5 text-green-800" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="m9 12 2 2 4-4"/></svg>

        <div class="ms-3">
            <h3 class="text-green-800 font-semibold">
                {!! $title !!}
            </h3>
            <p class="mt-2 text-sm text-green-700">
                {!! $text !!}
            </p>
        </div>
    </div>
</div>